classdef dcapoly < matlab.mixin.Copyable
    ...
    properties
        ...
        convexsolver_option = optimset('Algorithm', 'interior-point', 'Display','off'); % for knitro
    ...
    methods
        ...
        function status = optimize(obj)
            ...
            while obj.iter < obj.maxiter
                ...
                if strcmpi(obj.convexsolver,'bpppa')
                    ...
                    diagnostics=callconvexsolver_bis(obj.dcp.F.g,-yk,obj.dcp.C, xk, obj.convexsolver,obj.convexsolver_option);
                end
                ...
                if obj.linesearch == true
                    ...
                    d = xk1 - xk;
                    if (feasxk && feasxk1 && min(activesetxk - activesetxk1)>=0 && d'*obj.dcp.F.evalapproxgradf(xk1) < 0)
                        %if (normf<obj.boostthreshold && normx~=0)
                        if obj.usearmijo_adaptive
                            if obj.armojo_adaptive_method==0 % forwardtracking many times
                                [xacc,facc,obj.stepsize]=armijo_adaptive(obj,xk1,d,obj.stepsize); % increase many times
                            elseif obj.armojo_adaptive_method==1 % forwardtracking once
                                [xacc,facc,obj.stepsize]=armijo_adaptive_once(obj,xk1,d,obj.stepsize); % increase only once
                            else
                                [xacc,facc,obj.stepsize,consequtive_count]=armijo_adaptive_s(obj,xk1,d,obj.stepsize,consequtive_count); % increase only if two cons
                            end
                        else
                            % armijo
                            [xacc,facc]=armijo(obj,xk1,fk1,d,sqrt(2)/norm(d),0.5,1e-3,1e-8);
                        end
        ...
        % dca algorithm for solving dcp with starting point x0
        function status = optimize_old(obj)
            % dca optimizer
            % status.flag : 0 dca converges with tolx or tolf
            %               1 maxiter exceed
            %               2 problem infeasible or unbounded
...
% classical armijo rule
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [xx,ff] = armijo_old(obj,xk1,d)
    fk1=obj.dcp.F.evalf(xk1);
    nd = norm(d);
    sigma=1e-3;
    beta = 0.3;
